...
def rnn_cell_backward(da_next, cache):
    ...
    # 计算tanh相对于a_next的梯度.
    dtanh = (1 - np.square(a_next)) * da_next  # n_a

    # 计算关于Wax损失的梯度
    dxt = np.dot(Wax.T, dtanh)  # [n_a, n_x].T*n_a=n_x
    dWax = np.dot(dtanh, xt.T)  # n_a*n_x.T=[n_a,n_x]

    # 计算关于Waa损失的梯度
    da_prev = np.dot(Waa.T, dtanh)  # [n_a,n_a]*n_a=n_a
    dWaa = np.dot(dtanh, a_prev.T)  # n_a*n_a=[n_a,n_a]
