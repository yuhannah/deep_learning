...
def identity_block(X, f, filters, stage, block):
    ...
    # 主路径的第一部分
    ##卷积层
    # X = tf.keras.layers.Conv2D(F1, 1, 1, border_mode="valid", subsample=(1, 1), name=conv_name_base + "2a")(X)
    ...
    # 主路径的第二部分
    ##卷积层
    # X = tf.keras.layers.Conv2D(F2, f, f, border_mode="same", subsample=(1, 1), name=conv_name_base + "2b")(X)
    ...
    # 主路径的第三部分
    ##卷积层
    # X = tf.keras.layers.Conv2D(F3, 1, 1, border_mode="valid", subsample=(1, 1), name=conv_name_base + "2c")(X)
...
def convolutional_block(X, f, filters, stage, block, s=2):
    ...
    # 主路径
    ##主路径第一部分
    # X = tf.keras.layers.Conv2D(F1, 1, 1, border_mode='valid', subsample=(s, s))(X)
    ...
    ##主路径第二部分
    # X = tf.keras.layers.Conv2D(F2, f, f, border_mode='same', subsample=(1, 1))(X)
    X = tf.keras.layers.Conv2D(filters=F2, kernel_size=(f, f), strides=(1, 1), padding="same",
                               name=conv_name_base + "2b",
                               kernel_initializer=tf.keras.initializers.glorot_uniform(seed=0))(X)
    X = tf.keras.layers.BatchNormalization(axis=3, name=bn_name_base + "2b")(X)
    ...
    ##主路径第三部分
    # print(X[0].shape)
    # X = tf.keras.layers.Conv2D(F3, 1, 1, border_mode='valid', subsample=(1, 1))(X)
    X = tf.keras.layers.Conv2D(filters=F3, kernel_size=(1, 1), strides=(1, 1), padding="valid",
                               name=conv_name_base + "2c",
                               kernel_initializer=tf.keras.initializers.glorot_uniform(seed=0))(X)
    X = tf.keras.layers.BatchNormalization(axis=3, name=bn_name_base + "2c")(X)
    ...
    # X_shortcut = tf.keras.layers.Conv2D(F3, 1, 1, border_mode='valid', subsample=(s, s))(X_shortcut)
...
# my_image = tf.keras.applications.imagenet_utils.preprocess_input(my_image)
#
